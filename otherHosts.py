...
def custumOptionsValidation(val):
    ...
    for option in options:
        ...
        if opTmp and opTmp.count("option") == 1:
            # spaceCounter = opTmp.count(' ') ; print ("spaceCounter is = ", spaceCounter)
            op = opTmp.split(' ')[5]
            # print ("op = ", op)
    ...
    if ok == opLen:
        ...
        print (" OK LINES = %s / opLEN = %s " %(ok, opLen))
        return False
...
print ('<h1>&#160&#160&#160&#160&#160&#160&#160&#160&#160&#160&#160&#160'
      ...
      ' <font color=\"#FF4500\">'"      eNB (or Other) Hosts     "'</font></h1>')
...
print ('<p>' "debugging: form is: " + str(form) + '</p>'); # comment-out if Linux
...
if form.getvalue("valToSearch"):
    print ('<form method="post" action="otherHosts.py">')
        ...
        if searchListFunc(val2Search, hostDescList):
            for index, item in enumerate(hostDescList):
                if val2Search in item:
                    if int(hostTypeList[index]) == 3: printRowFound(index)
...
if form.getvalue("hostMacChosen"):
    ...
    if checkMAC(hostMac):
        ...
        if hostMac in hostsMacList:
            ...
            #    print (' --- DHCP Client Identifier:<input type="text" name="dhcpClientId" value= ' + str(dhcpClientIdentifier[ind]) + ' ''</p>')
            print ('<p> --- CUSTOM OPTIONS (43)-----:  </p>')
            print ('<textarea name = "textcontent" cols = "60" rows = 12">' + ("".join(str(val.replace('\r\n','')) for val in HostOptions[ind])).replace('\r\n','') +  '</textarea>')
   ...
   if custumOptionsValidation(customOptions):
       if not form.getvalue("newHostMac") in hostsMacList and not any(form.getvalue("newHostMac") in subl for subl in ipsecHostsMacList):   
           if examSpecialCharInStr(hostDesc) and examSpecialCharInStr(customOptions):
               ...
               if checkIpExistance(ipAddrChos,ind) and checkDescriptionExist(hostDesc,ind):
                   if validate_ipaddress(form.getvalue('ipAddr')): 
                       ...
                       if imsiFormatValid(dhcpClientIden) and duplicateImsi(dhcpClientIden,ind):
                           ...
                           # print ("isEnbOfRelay = ", form.getvalue('isEnbOfRelay'))
                           if form.getvalue('isEnbOfRelay'):
                           ...
                           f3 = open(filename, 'r')
                           # Create New Host Mac
                           if hostMac not in hostsMacList: # adding new host
                               #print (" Adding new host mac to file ")
                               firstHostFound=0
                               for lin in f3:
                               ...
                               #       '</td><td><input type = "submit" name = "valToSearch" value =' +str(hostMac)+ ' </td></tr><tr>')
                               #print ("</tr></table>")
                           elif OrigHostDesc:
                               inLin='0'
                               for lin in f3:
                           ...
                           '''
                           print ('<h2> ## Host Mac # ' + str(hostMac) + ' # New Values Are: </h2> ')
...
if not form.getvalue("hostMacChosen") and not form.getvalue("valToSearch") and not form.getvalue("SubmitChanges") and not form.getvalue("Delete?") and not form.getvalue('newHost?'):
    print ('<form method="post" action="otherHosts.py">')
    # print ('<p> ## =========================================================================== ## </p>')
    print ('<font color=\"#0000FF\">&#160# Search Host by ( MAC / IP / DESCRIPTION ): <input type="text" name="valToSearch"/></font>')
    ...
    print ("<font color=\"#FF4500\"><h2> eNB (Other) Hosts List: </h2></font>")
    divider = 22
    divL = int(len(hostsMacList)/divider ) ; #print  ("divL = ", divL)
    divH = math.ceil( (len(hostsMacList)/divider )) ; #print ("divH rounded up = ", divH)
    for i in range(1, divL+1):
        ...
        if i == (divL):
            RL1 = (int(i)*divider) ; RH1 = ((int(i+1)*divider)-((int(i+1)*divider)-(len(hostsMacList)))-3)
            print ('<input type = "submit" name = "tableRange" value ='"lRange"+str(RL1)+'-'+str(RH1)+'>')
    print ("<table><tr><th>| # |</th><th>| MAC |</th><th>| DESCIRPTION |</th><th>| IP-ADDRESS |</th><th>| EDIT LINK |</th></tr><tr>")
    if form.getvalue("tableRange"):
        ...
        for y in range(int(RL), int(RH)+1):
            print ('<td>' +str(y)+ '</td><td>'+str(hostsMacList[y])+'</td> <td>'+str(hostDescList[y])+'</td> <td>'+str(ipAddrList[y])+
        ...
        for y, Mac in enumerate(hostsMacList):
            print ('<td>' +str(y)+ '</td><td>'+str(hostsMacList[y])+'</td> <td>'+str(hostDescList[y])+'</td><td>'+str(ipAddrList[y])+ '</td>'
                   '</td><td><input type = "submit" name = "hostMacChosen" value =' +str(hostsMacList[y])+ ' </td></tr>')
